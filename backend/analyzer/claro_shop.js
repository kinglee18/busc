...
function findProd(tx,arr1,arr2) {
    //console.log(JSON.stringify(arr2,undefined,2))
    let ctg = [];
    let bn = [];
    for(let op of arr1) {
    ...
    if(ctg.length == 0) {
        let nv = cls.clearAllStopWords(tx);
        let palabras = nv.split(' ');
        for(let palabra of palabras) {
            for(let op of arr1) {
                for(let op2 of op._source.relacion) {
                    let reg = new RegExp("\\b"+palabra+"\\b");
                    let matches = op2.match(reg);
                    if(matches) {
                        let ind = prob.probWords(op2,palabra);
                        //console.log('Probalidad: '+ind+'----'+op2);
                        //let inicio = op2.split(' ')[0];
                        if(!ctg.includes(op2) && ind) ctg.push(op2);
                        //if(!marcas.includes(op._source.valor)) marcas.push(op._source.valor);
                    }
                }
            }
    ...
    if(bn.length == 0) {
        ...
        for(let palabra of palabras) {
            ...
            for(let op of arr2) {
                let titu = cls.clearTexto(op._source.title);
                let description = cls.clearTexto(op._source.description);
                //console.log('==>('+titu+')====('+description+')')
                let reg = new RegExp("("+palabra+")");
                let matches = titu.match(reg);
                if(matches) {
                    if(!bn.includes(palabra)) bn.push(palabra)
                    //if(!marcas.includes(brand)) marcas.push(brand);
                }
    
                let reg2 = new RegExp("("+palabra+")");
                let matches2 = description.match(reg2);
                if(matches2) {
                    if(!bn.includes(palabra)) bn.push(palabra)
                    //if(!marcas.includes(brand)) marcas.push(brand);
                }
